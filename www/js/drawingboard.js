...
DrawingBoard.Board.prototype = {
    ...
    customAlert: function(){
		//alert('you just called custom function');
		//console.log(this.coords.current);
		return this.coords.current;
	},
	...
	resetBackground: function(background, callback) {
		...
		if (bgIsColor) {
			...
			var imageAndStroke = [];
			this.stroke = [];
			imageAndStroke[0] = this.stroke; 
			imageAndStroke[1] = this.getImg();
			this.history.initialize(imageAndStroke);
	...
	initHistory: function() {
		this.history = new SimpleUndo({
			...
			provider: $.proxy(function(done) {
				...
				//save history every time an image is drawn on the board
				
				endX = this.customAlert().x;
				endY = this.customAlert().y;		
				//if clear is pressed then do not add stroke and clear result screen instead
				if(!clearFlag){
				...
				var imageAndStroke = [];
				imageAndStroke[0] = potentialKanji.getLoadingStroke();
				imageAndStroke[1] = this.getImg();
	...
	setImg: function(src, opts, stroke) {
		opts = $.extend({
